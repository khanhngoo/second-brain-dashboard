...
def _make_past_block(conn):
    t = core.create_task(conn, pillar="cracked_engineer", title="deep work")
...
def test_future_block_untouched(seeded_db, frozen_clock):
    t = core.create_task(seeded_db, pillar="cracked_engineer", title="later")
    blk = core.create_time_block(
...
def test_skip_voids_session_and_removes_pillar_minutes(seeded_db, frozen_clock):
    ...
    mins_before = {r["slug"]: r["minutes"] for r in before}["cracked_engineer"]
    ...
    mins_after = {r["slug"]: r["minutes"] for r in after}["cracked_engineer"]
    assert mins_after == 0
